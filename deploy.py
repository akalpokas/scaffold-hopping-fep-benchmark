...
def submit_jobs(network_path: Path, specific_edge: str = None, protocol: str = "prod", leg_name: str = "both", replicate: int = None, de_strength: int = 150):
    ...
    legs_to_run = [leg_name.lower()] if leg_name.lower() in ["free", "bound"] else ["free", "bound"]
    ...
    print(f"Edges: {len(edges_data)} | Legs: {legs_to_run} | Replicates: {replicates_to_run}\n")
        ...
        for leg in legs_to_run:
            ...
            for rep in replicates_to_run:
                ...
                try:
                    ...
                    print(f"Submitted {edge_id} | {leg.upper()} | Rep {rep} -> {result.stdout.strip()}")
                    
                    # Brief pause to avoid overwhelming the SLURM scheduler daemon
                    time.sleep(0.2)
                    
                except subprocess.CalledProcessError as e:
                    print(f"Failed {edge_id} | {leg.upper()} | Rep {rep} -> Error: {e.stderr}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--edge-id", default=None, type=str, help="Optional: Submit only a single edge")
    
    parser.add_argument("--leg_name", default="both", type=str, choices=["free", "bound", "both"], 
    ...
    parser.add_argument("--de_strength", type=int, default=150, help="Dissocation energy parameter for the run")
