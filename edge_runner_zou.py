...
def get_user_input():
    ...
    parser.add_argument(
        ...
        type=str,
        default="10",
        help="Restraint strength for aux restraints",
    )
...
def main():
    ...
    if isinstance(config, (RBFEEdge, RHFEEdge)):
        if "bond annihilation" in metadata_notes:
            somd2_config.lambda_schedule = "ring_break_morph_zou"
            bond_alchemy = True
    ...
    if bond_alchemy:
        ...
        # Begin edge specific restraint setup using Zou et al approach
        sire_system = sr.morph.link_to_reference(sire_system)
        if args.edge_id == "cyclohexane_to_hexane":
        ...
        elif args.edge_id == "cyclopentane_to_pentane":
            restraints = sr.restraints.dihedral(
                mols=sire_system,
                atoms=sire_system.dihedrals()[2],
                kphi=f"{args.restraint_strength} kcal mol-1 rad-2",
            ...
            restraint1 = sr.restraints.dihedral(
                mols=sire_system,
                atoms=sire_system.dihedrals()[3],
                kphi=f"{args.restraint_strength} kcal mol-1 rad-2",
            )

            restraints.add(restraint1)

        elif args.edge_id == "cyclobutane_to_butane":
        ...
        elif args.edge_id == "cyclopropane_to_propane":
            restraints = sr.restraints.angle(
                mols=sire_system,
                atoms=sire_system.angles()[2],
    ...
    if bond_alchemy:
        out_name = f"{args.leg_name}_{mods_prefix}zou_restraint_{int(args.restraint_strength)}_{args.protocol}_repl_{args.replicate}"
    else:
