...
# Set up a basic logger to catch the warnings/debugs from your helper functions
...
def _analyse_somd2(work_dir, T=300.0, use_checkpoint_files=False, **kwargs):
    ...
    # Note: Assumes 'sr' (Sire) and 'BSS' (BioSimSpace) are available in your environment
    ...
    if use_checkpoint_files:
        ...
        for f in files:
            ...
            # Ensure 'sr' is imported at the top of your actual script/environment
            import sire as sr 
        ...
        for f in files:
            path = Path(f)
            # Ensure 'BSS' is imported at the top of your actual script/environment
    ...
    if len(set(lengths)) != 1:
        logger.warning(f"Not all dataframes contain the same number of entries: {lengths}")

    return analysed_df_list


def _plot_alchemlyb_convergence(analysed_df_list, estimator="MBAR", units="kcal/mol", number_of_points=10):
    """
    ...
    convergence_df = forward_backward_convergence(analysed_df_list, estimator=estimator, num=number_of_points)
    ...
    return convergence_df # Fixed typo here from 'convergence_d'
...
class ConvergenceAnalyzer:
    ...
    def analyze_replicate(self, edge_id: str, run_dir: Path, out_dir: Path, leg: str, rep: int):
        ...
        print(f"    Analysed dataframes for Replicate {rep}: {[df.shape for df in analysed_df_list]}")
        
        # Auto-detect global maximum time
        ...
        elif global_max_ps != self.expected_max_ps[leg_key] and not self.override_mismatch:
            raise ValueError(
        ...
        for base_name, bounds in self.base_strategies.items():
            ...
            # Apply the exact time mask for this strategy
            ...
            start_time = int(trimmed_analysed_df_list[0][0].index.get_level_values("time").min())
            start_ns = start_time / 1000

            # Create title label
            end_ns_label = round(bounds["t_max"] / 1000) if bounds["t_max"] is not None else global_max_ns
            strategy_name = f"{base_name} ({round(start_ns)} - {end_ns_label} ns)"

            # Run alchemlyb calculation using the helper function
            convergence_df = _plot_alchemlyb_convergence(trimmed_analysed_df_list, number_of_points=10)

            # Normalize data_fraction column
            convergence_df["data_fraction"] = convergence_df["data_fraction"] / convergence_df["data_fraction"].max()
            
            plt.close('all') # Silently close the unwanted figure(s) generated by alchemlyb
    ...
    def aggregate_leg(self, edge_id: str, out_dir: Path, leg: str):
        ...
        if not csv_files:
            print(f"    [!] No convergence data found to aggregate for {edge_id} {leg}.")
            return

        combined_dfs = pd.concat([pd.read_csv(f) for f in csv_files], ignore_index=True)

        # 3. SUMMARY STATISTICS
        final_rows = combined_dfs.sort_values("data_fraction").groupby(["strategy", "replicate"]).tail(1)
        summary_df = final_rows.groupby("strategy").agg(
        ...
        for ax, strategy in zip(axes, strategy_labels):
            ...
            for rep in subset_df["replicate"].unique():
                ...
                ax.plot(rep_df["data_fraction"], rep_df["Forward"], color="#007FFF", alpha=0.25, linewidth=1, zorder=1)
                
                # Backward individual replicate
                ax.plot(rep_df["data_fraction"], rep_df["Backward"], color="#FF4500", alpha=0.25, linewidth=1, zorder=1)

        # Plot aggregated mean lines using the numerical data_fraction on the x-axis
            sns.lineplot(
                x="data_fraction", y="Forward", data=subset_df, ax=ax, 
                linewidth=2.5, color="#007FFF", label="Forward Mean", 
                errorbar="sd", marker="o", markersize=6, zorder=2
            )

            sns.lineplot(
                x="data_fraction", y="Backward", data=subset_df, ax=ax, 
                linewidth=2.5, color="#FF4500", label="Backward Mean", 
                errorbar="sd", marker="s", markersize=6, zorder=2
            )
        ...
        plt.suptitle(f"ΔG Convergence: {edge_id.capitalize()} ({leg.capitalize()})", weight='bold', y=1.05, fontsize=18)
        plt.tight_layout()
